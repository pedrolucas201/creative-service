...
func (c *Client) UploadVideo(ctx context.Context, adAccountID, name, fileName string, mp4 []byte) (string, error) {
	...
	if out.ID == "" {
		return "", errors.New("upload video: empty id")
	}
	return out.ID, nil
}
...
func (c *Client) CreateCreative(ctx context.Context, adAccountID string, payload map[string]any) (string, error) {
	...
	if out.ID == "" {
		return "", errors.New("create creative: empty id")
	}
	return out.ID, nil
}
...
func (c *Client) GetObject(ctx context.Context, objectID string, fields []string) (map[string]any, error) {
	q := url.Values{}
	if len(fields) > 0 {
		q.Set("fields", strings.Join(fields, ","))
	}
	...
	var out map[string]any
	if err := c.doJSON(ctx, http.MethodGet, objectID, q, nil, &out); err != nil {
		return nil, err
	}
	return out, nil
}
...
func (c *Client) CreateCampaign(ctx context.Context, adAccountID string, payload map[string]any) (string, error) {
	...
	if out.ID == "" {
		return "", errors.New("create campaign: empty id")
	}
	return out.ID, nil
}
...
func (c *Client) CreateAdSet(ctx context.Context, adAccountID string, payload map[string]any) (string, error) {
	...
	if out.ID == "" {
		return "", errors.New("create adset: empty id")
	}
	return out.ID, nil
}
...
func (c *Client) CreateAd(ctx context.Context, adAccountID string, payload map[string]any) (string, error) {
	...
	if out.ID == "" {
		return "", errors.New("create ad: empty id")
	}
	return out.ID, nil
}
